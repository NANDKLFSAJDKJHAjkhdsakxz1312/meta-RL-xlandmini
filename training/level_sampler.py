...
class LevelSampler:
    ...
    def sample_new_level(self):
        rng, sub_rng = jax.random.split(self.rng)
        unseen_levels = [l for l in range(self.total_levels) if l not in self.seen_levels]
    ...
    def sample_replay_level(self):
        ...
        replay_level = jax.random.choice(sub_rng, list(self.seen_levels), p=priorities)  # 注意，这里要将 OrderedSet 转换为列表
        return replay_level
