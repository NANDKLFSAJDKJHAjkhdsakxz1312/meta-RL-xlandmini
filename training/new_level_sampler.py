...
class LevelSampler:
    ...
    def initialize(self, pholder_level: int, pholder_level_extra=None) -> Sampler:
        """
        ...
        Args:
            pholder_level (int): A placeholder level that will be used to initialize the level buffer.
        ...
        """
        sampler = {
            "levels": jnp.array([pholder_level]).repeat(self.capacity, axis=0),
    ...
    def sample_replay_level(self, sampler: Sampler, rng: chex.PRNGKey) -> Tuple[Sampler, Tuple[int, int]]:
        """
        ...
        Returns:
            Tuple[Sampler, Tuple[int, int]]: The updated sampler object, the sampled level's index and the level itself.
        """
    ...
    def sample_replay_levels(self, sampler: Sampler, rng: chex.PRNGKey, num: int) -> Tuple[Sampler, Tuple[chex.Array, int]]:
        """
        ...
        Returns:
            Tuple[Sampler, Tuple[chex.Array, int]]: The updated sampler, an array of indices, and multiple levels.
        """
        return jax.lax.scan(self.sample_replay_level, sampler, jax.random.split(rng, num), length=num)
    
    def insert(self, sampler: Sampler, level: int, score: float, level_extra: dict=None) -> Tuple[Sampler, int]:
        """
    ...
    def insert_batch(self, sampler: Sampler, levels: int, scores: chex.Array, level_extras: dict=None) -> Tuple[Sampler, chex.Array]:
        """
    ...
    def get_levels(self, sampler: Sampler, level_idx: int) -> int:
        """
    ...
    def update_batch(self, sampler: Sampler, level_inds: chex.Array, scores: chex.Array, level_extras: dict=None) -> Sampler:
        ...
        Args:
            sampler (Sampler): The sampler object
            level_inds (chex.Array): int indices
    ...
    def _insert_new(self, sampler: Sampler, level: int, score: float, level_extra: dict) -> Tuple[Sampler, int]:
...
def make_level_generator(n_levels:int)->Callable[[chex.PRNGKey],int]:
    def sample(rng:chex.PRNGKey)->int:
        level_id = jax.random.randint(rng,shape=(),minval=0,maxval=n_levels)
        return level_id
    return sample
