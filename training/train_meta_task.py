...
def make_states(config: TrainConfig):
    ...
    pholder_level_batch = jax.tree_map(
        lambda x: jnp.array([x]).repeat(config.num_envs, axis=0), pholder_level
    )
        ...
        def _meta_step(meta_state, _):
            ...
            def on_replay_levels(rng: chex.PRNGKey, train_state: TrainState):
                ...
                sampler, (level_inds, levels) = level_sampler.sample_replay_levels(
                    sampler, _rng1, config.num_envs_per_device
                )
                ...
                ########
                rulesets = jax.vmap(benchmark.get_ruleset)(
                    levels
                )  # change the random ruleset_rng to levels
                meta_env_params = env_params.replace(ruleset=rulesets)

                timestep = jax.vmap(env.reset, in_axes=(0, 0))(
                    meta_env_params, reset_rng
                )
                ...
                def _update_step(runner_state, _):
                    ...
                    def _env_step(runner_state, _):
                        ...
                        dist, value, hstate = train_state.apply_fn(
                            train_state.params,
                            {
                                ...
                                "obs_dir": prev_timestep.observation["direction"][
                                    :, None
                                ],
                    ...
                    max_returns = jnp.maximum(
                        level_sampler.get_levels_extra(sampler, level_inds)[
                            "max_return"
                        ],
                        compute_max_returns(transitions.done, transitions.reward),
                    )
            ...
            def on_new_levels(rng: chex.PRNGKey, train_state: TrainState):
                ...
                new_levels = jax.vmap(sample_random_level)(
                    jax.random.split(_rng1, config.num_envs_per_device)
                )
                rulesets = jax.vmap(benchmark.get_ruleset)(
                    new_levels
                )  # change the random ruleset_rng to levels
                ...
                def _update_step(runner_state, _):
                    ...
                    sampler = train_state.sampler
                    max_returns = compute_max_returns(
                        transitions.done, transitions.reward
                    )
                    # scores = compute_score(config.score_function, transitions.done, transitions.value, max_returns, advantages)
                    scores = jnp.mean(jnp.abs(advantages), axis=0)
