...
class DoorKey(Environment[EnvParams, EnvCarry]):
    ...
    def _generate_problem(self, params: EnvParams, key: jax.Array) -> State[EnvCarry]:
        ...
        wall_pos = jax.random.randint(
            keys[0], shape=(), minval=2, maxval=params.width - 2
        )
        door_pos = jax.random.randint(
            keys[1], shape=(), minval=1, maxval=params.width - 2
        )
        ...
        grid = vertical_line(
            ...
            0,
            params.height,
            tile=TILES_REGISTRY[Tiles.WALL, Colors.GREY],
        )
        grid = grid.at[door_pos, wall_pos].set(
            TILES_REGISTRY[Tiles.DOOR_LOCKED, Colors.YELLOW]
        )
